 ...
 */
static void ds1338_virt_incr_addr(ds1338_virt_t* const p) {
 ...
 */
static void ds1338_virt_update(const ds1338_virt_t* const p) {
 ...
 * in 4 digit format.
 */
static uint8_t ds1338_virt_days_in_month(uint8_t month, uint16_t year) {
 ...
 */
static uint8_t ds1338_virt_tick_bcd_reg(bcd_reg_t* bcd_reg) {
  // Unpack BCD
  uint8_t x =
      (*bcd_reg->reg & 0x0F) + 10 * ((*bcd_reg->reg & bcd_reg->tens_mask) >> 4);
 ...
 */
static void ds1338_virt_tick_time(ds1338_virt_t* p) {
  /*
   * Seconds
   */
  bcd_reg_t reg = {.reg = &p->nvram[DS1338_VIRT_SECONDS],
                   .min_val = 0,
                   .max_val = 59,
  ...
  if (ds1338_get_flag(p->nvram[DS1338_VIRT_HOURS], DS1338_VIRT_12_24_HR)) {
    ...
    uint8_t pm =
        ds1338_get_flag(p->nvram[DS1338_VIRT_HOURS], DS1338_VIRT_AM_PM);
    cascade = ds1338_virt_tick_bcd_reg(&reg);
    if (cascade) {
      if (pm) {
        // Switch to AM
  ...
  reg.max_val =
      ds1338_virt_days_in_month(UNPACK_BCD(p->nvram[DS1338_VIRT_MONTH]), year);
  reg.tens_mask = 0b00110000;
  cascade = ds1338_virt_tick_bcd_reg(&reg);
  if (!cascade)
    return;
...
static void ds1338_virt_cycle_square_wave(ds1338_virt_t* p) {
  if (!ds1338_get_flag(p->nvram[DS1338_VIRT_CONTROL], DS1338_VIRT_SQWE)) {
    if (p->verbose) printf("DS1338: SQWE disabled\n");
 ...
 */
static void ds1338_print_time(ds1338_virt_t* p) {
  ...
  if (p->verbose)
    printf(
        "Time: %02i:%02i:%02i  Day: %i Date: %02i:%02i:%02i PM:%01x\n",
        hours,
        minutes,
        seconds,
        day,
...
ds1338_virt_clock_tick(struct avr_t* avr, avr_cycle_count_t when, void* ptr) {
  auto p = (ds1338_virt_t*)ptr;
  avr_cycle_count_t next_tick =
      when + avr_usec_to_cycles(avr, DS1338_CLK_PERIOD_US / 2);
  ...
  uint8_t prescaler_mode =
      ds1338_get_flag(p->nvram[DS1338_VIRT_CONTROL], DS1338_VIRT_RS0) +
      (ds1338_get_flag(p->nvram[DS1338_VIRT_CONTROL], DS1338_VIRT_RS1) << 1);
...
static void ds1338_virt_clock_xtal_init(struct avr_t* avr, ds1338_virt_t* p) {
  p->rtc = 0;
  ...
  avr_cycle_timer_register_usec(
      avr, DS1338_CLK_PERIOD_US / 2, ds1338_virt_clock_tick, p);

#if 0
  printf(
      "DS1338 clock crystal period %duS or %d cycles\n",
 ...
 */
static void
ds1338_virt_in_hook(struct avr_irq_t* irq, uint32_t value, void* param) {
  ...
  if (v.u.twi.msg & TWI_COND_STOP) {
    ...
    /* We should not zero the register address here because read mode uses the
     * last
  ...
  if (p->selected) {
    ...
    if (v.u.twi.msg & TWI_COND_WRITE) {
      ...
      if (p->reg_selected) {
        if (p->verbose)
          printf(
              "DS1338 set register 0x%02x to 0x%02x\n",
              p->reg_addr,
              v.u.twi.data);
    ...
    if (v.u.twi.msg & TWI_COND_READ) {
      if (p->verbose)
        printf(
            "DS1338 READ data at 0x%02x: 0x%02x\n",
            p->reg_addr,
            p->nvram[p->reg_addr]);
...
/*
 * Initialise the DS1388 virtual part. This should be called before anything
 * else.
 */
void ds1338_virt_init(struct avr_t* avr, ds1338_virt_t* p) {
  memset(p, 0, sizeof(*p));
  ...
  p->irq =
      avr_alloc_irq(&avr->irq_pool, 0, DS1338_IRQ_COUNT, _ds1338_irq_names);
  avr_irq_register_notify(p->irq + TWI_IRQ_OUTPUT, ds1338_virt_in_hook, p);

  // Start with the oscillator disabled, at least until there is some "battery
  // backup"
 ...
 */
void ds1338_virt_attach_twi(ds1338_virt_t* p, uint32_t i2c_irq_base) {
  avr_connect_irq(
      p->irq + TWI_IRQ_INPUT,
 ...
 */
void ds1338_virt_attach_square_wave_output(
    ds1338_virt_t* p,
    ds1338_pin_t* wiring) {
  avr_connect_irq(
      p->irq + DS1338_SQW_IRQ_OUT,
      avr_io_getirq(
          p->avr, AVR_IOCTL_IOPORT_GETIRQ(wiring->port), wiring->pin));
}
